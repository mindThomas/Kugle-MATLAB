...
vicon = RemoveTime(vicon, 120.75, 120.79); % remove glitch

[data, vicon] = TrimSynced(data, vicon, 14.2+50, 100); % drag_test
%[data, vicon] = TrimSynced(data, vicon, 14.4, 40); % station_keeping_nonaggressive_with_initial
%[data, vicon] = TrimSynced(data, vicon, 30+6.5, 10); % station_keeping_nonaggressive
...
drag_times = [18.08, 26.94, 33.53, 54.51, 75.31];
release_times = [19.28, 28.61, 37.28, 58.73, 81];
release_times = [18.78, 28.13, 36.86, 58.95, 80.54];

%% Velocity
fig = figure(1); set(fig, 'NumberTitle', 'off', 'Name', 'Velocity');
ax1 = subplot(2,1,1); plot(vicon.time, vicon.velocity(:,1), data.time, data.velocity(:,1), data.time, data.velocity_ref_inertial(:,1)); ylabel('m/s'); title('dx'); legend('Vicon', 'VEKF', 'Reference'); vline(drag_times,'k--');  vline(release_times,'g--');
ax2 = subplot(2,1,2); plot(vicon.time, vicon.velocity(:,2), data.time, data.velocity(:,2), data.time, data.velocity_ref_inertial(:,2)); ylabel('m/s'); title('dy'); legend('Vicon', 'VEKF', 'Reference'); vline(drag_times,'k--');  vline(release_times,'g--');
linkaxes([ax1,ax2],'x');
xlabel('Time [s]');

%% Position
fig = figure(2); set(fig, 'NumberTitle', 'off', 'Name', 'Position');
ax1 = subplot(2,1,1); plot(vicon.time, vicon.position(:,1), data.time, data.position(:,1)); ylabel('m'); title('x'); legend('Vicon', 'Estimated'); vline(drag_times,'k--');  vline(release_times,'g--');
ax2 = subplot(2,1,2); plot(vicon.time, vicon.position(:,2), data.time, data.position(:,2)); ylabel('m'); title('y'); legend('Vicon', 'Estimated'); vline(drag_times,'k--');  vline(release_times,'g--');
linkaxes([ax1,ax2],'x');
xlabel('Time [s]');
...
csvwrite_with_headers('drag_test_vicon.csv', out, headers);
...
csvwrite_with_headers('drag_test_system.csv', out, headers);

% Store drag times
csvwrite_with_headers('drag_test_dragtimes.csv', {drag_times', release_times'}, {'drag','release'});