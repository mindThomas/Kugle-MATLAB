...
[data, vicon] = TrimSynced(data, vicon, 20.5+1.24, 75.3-20.5-1.24);
...
fig = figure(5); set(fig, 'NumberTitle', 'off', 'Name', 'Orientation tracking error');
...
csvwrite_with_headers('increasing_sine_roll_test_vicon.csv', out, headers);

out = [data.time, data.velocity, rad2deg(data.roll), rad2deg(data.pitch), rad2deg(data.yaw), rad2deg(roll_ref), rad2deg(pitch_ref), rad2deg(yaw_ref), data.q, data.omega_body, data.omega_ref_body, data.S, data.torque, data.torque_delivered];
out = Downsample(out, 2); % downsample plot data
...
csvwrite_with_headers('increasing_sine_roll_test_system.csv', out, headers);