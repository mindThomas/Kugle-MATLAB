...
[data, vicon] = TrimSynced(data, vicon, 25, 57.5);
...
subplot(2,1,1); plot(vicon.time, vicon.velocity(:,1), data.time, velocity_kinematics_est(:,1), data.time, velocity_kinematics_mti(:,1), data.time, velocity_kinematics_upright(:,1)); ylabel('m/s'); title('dx'); legend('Vicon', 'Kinematics using estimated Quaternion', 'Kinematics using MTI Quaternion', 'Kinematics without Quaternion');
subplot(2,1,2); plot(vicon.time, vicon.velocity(:,2), data.time, velocity_kinematics_est(:,2), data.time, velocity_kinematics_mti(:,2), data.time, velocity_kinematics_upright(:,2)); ylabel('m/s'); title('dy'); legend('Vicon', 'Kinematics using estimated Quaternion', 'Kinematics using MTI Quaternion', 'Kinematics without Quaternion');
xlabel('Time [s]');
...
subplot(2,1,1); plot(data.time, velocity_kinematics_mti(:,1)-velocity_kinematics_upright(:,1)); ylabel('m/s'); title('dx difference');
subplot(2,1,2); plot(data.time, velocity_kinematics_mti(:,2)-velocity_kinematics_upright(:,2)); ylabel('m/s'); title('dy difference');
xlabel('Time [s]');
...
out = [data.time, dtheta(:,1), dtheta(:,2), dtheta(:,3), velocity_kinematics_mti(:,1), velocity_kinematics_mti(:,2)];
out = Downsample(out, 2); % downsample plot data
headers = {'time', 'dtheta0', 'dtheta1', 'dtheta2', 'kinematics_velocity_x', 'kinematics_velocity_y'};
