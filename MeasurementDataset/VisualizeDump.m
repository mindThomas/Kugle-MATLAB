...
PlotWheelSlipDetection = false;
PlotCOM = false;
PlotMotorTorque = false;
PlotSlidingManifold = false;
PlotMotorCurrents = false;
PlotVelocityControllerInfo = false;
...
if (PlotQuaternionEstimate)
    ...
    ax1 = subplot(4,1,1); plot(data.time, data.q(:,1), data.time, data.q_ref(:,1), data.time, data.mti_q(:,1), vicon.time, vicon.q(:,1)); title('q.w'); legend('Estimate', 'Reference', 'MTI', 'Vicon');
    ax2 = subplot(4,1,2); plot(data.time, data.q(:,2), data.time, data.q_ref(:,2), data.time, data.mti_q(:,2), vicon.time, vicon.q(:,2)); title('q.x'); legend('Estimate', 'Reference', 'MTI', 'Vicon');
    ax3 = subplot(4,1,3); plot(data.time, data.q(:,3), data.time, data.q_ref(:,3), data.time, data.mti_q(:,3), vicon.time, vicon.q(:,3)); title('q.y'); legend('Estimate', 'Reference', 'MTI', 'Vicon');
    ax4 = subplot(4,1,4); plot(data.time, data.q(:,4), data.time, data.q_ref(:,4), data.time, data.mti_q(:,4), vicon.time, vicon.q(:,4)); title('q.z'); legend('Estimate', 'Reference', 'MTI', 'Vicon');
...
if (PlotEulerAnglesEstimate)
    ...
    ax1 = subplot(3,1,1); plot(data.time, rad2deg(data.roll), data.time, rad2deg(data.roll_ref), data.time, rad2deg(data.mti_roll), vicon.time, rad2deg(vicon.roll)); title('Roll');  legend('Estimate', 'Reference', 'MTI', 'Vicon'); ylabel('deg');
    ax2 = subplot(3,1,2); plot(data.time, rad2deg(data.pitch), data.time, rad2deg(data.pitch_ref), data.time, rad2deg(data.mti_pitch), vicon.time, rad2deg(vicon.pitch)); title('Pitch');  legend('Estimate', 'Reference', 'MTI', 'Vicon'); ylabel('deg');
    ax3 = subplot(3,1,3); plot(data.time, rad2deg(data.yaw), data.time, rad2deg(data.yaw_ref), data.time, rad2deg(data.mti_yaw), vicon.time, rad2deg(vicon.yaw)); title('Yaw');  legend('Estimate', 'Reference', 'MTI', 'Vicon'); ylabel('deg');
    linkaxes([ax1,ax2,ax3],'x');
    xlabel('Time [s]');   
end
...
if (PlotQuaternionDerivativeEstimate)
    ...
    ax1 = subplot(4,1,1); plot(data.time, data.dq(:,1), data.time, data.mti_dq(:,1), vicon.time, vicon.dq(:,1)); title('dq.w'); legend('Estimate', 'MTI', 'Vicon');
    ax2 = subplot(4,1,2); plot(data.time, data.dq(:,2), data.time, data.mti_dq(:,2), vicon.time, vicon.dq(:,2)); title('dq.x'); legend('Estimate', 'MTI', 'Vicon');
    ax3 = subplot(4,1,3); plot(data.time, data.dq(:,3), data.time, data.mti_dq(:,3), vicon.time, vicon.dq(:,3)); title('dq.y'); legend('Estimate', 'MTI', 'Vicon');
    ax4 = subplot(4,1,4); plot(data.time, data.dq(:,4), data.time, data.mti_dq(:,4), vicon.time, vicon.dq(:,4)); title('dq.z'); legend('Estimate', 'MTI', 'Vicon');
    linkaxes([ax1,ax2,ax3,ax4],'x');
    xlabel('Time [s]');    
end
...
if (PlotAngularVelocityEstimate)
    ...
    ax1 = subplot(3,1,1); plot(data.time, data.gyroscope(:,1), data.time, data.omega_body(:,1), data.time, data.mti_omega_body(:,1), data.time, data.omega_ref_body(:,1), vicon.time, vicon.omega_body(:,1)); title('omega.x'); legend('Gyroscope', 'Estimate', 'MTI', 'Reference', 'Vicon'); ylabel('rad/s');
    ax2 = subplot(3,1,2); plot(data.time, data.gyroscope(:,2), data.time, data.omega_body(:,2), data.time, data.mti_omega_body(:,2), data.time, data.omega_ref_body(:,2), vicon.time, vicon.omega_body(:,2)); title('omega.y'); legend('Gyroscope', 'Estimate', 'MTI', 'Reference', 'Vicon'); ylabel('rad/s');
    ax3 = subplot(3,1,3); plot(data.time, data.gyroscope(:,3), data.time, data.omega_body(:,3), data.time, data.mti_omega_body(:,3), data.time, data.omega_ref_body(:,3), vicon.time, vicon.omega_body(:,3)); title('omega.z'); legend('Gyroscope', 'Estimate', 'MTI', 'Reference', 'Vicon'); ylabel('rad/s');
    linkaxes([ax1,ax2,ax3],'x');
end
...
if (PlotPositionAndVelocityEstimate)
    ...
    ax2 = subplot(3,1,2); plot(data.time, data.velocity(:,1), data.time, kinematics_velocity(:,1), data.time, kinematics_CoR_velocity(:,1), data.time, data.velocity_ref_inertial(:,1), vicon.time, vicon.velocity(:,1)); legend('Estimate', 'Kinematics Ball', 'Kinematics CoR', 'Reference', 'Vicon'); title('Velocity X'); ylabel('m/s');
    ax3 = subplot(3,1,3); plot(data.time, data.velocity(:,2), data.time, kinematics_velocity(:,2), data.time, kinematics_CoR_velocity(:,2), data.time, data.velocity_ref_inertial(:,2), vicon.time, vicon.velocity(:,2)); legend('Estimate', 'Kinematics Ball', 'Kinematics CoR', 'Reference', 'Vicon'); title('Velocity Y'); ylabel('m/s');
    linkaxes([ax1,ax2,ax3],'x');
    xlabel('Time [s]');    
end
