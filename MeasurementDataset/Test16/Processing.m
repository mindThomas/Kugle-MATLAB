...
[data, vicon] = TrimSynced(data, vicon, 23, 42.4);
...
csvwrite_with_headers('sine_roll_test_vicon.csv', out, headers);

out = [data.time, data.velocity, rad2deg(data.roll), rad2deg(data.pitch), rad2deg(data.yaw), rad2deg(roll_ref), rad2deg(pitch_ref), rad2deg(yaw_ref), data.q, data.omega_body, data.omega_ref_body, data.S, data.torque];
%out = Downsample(out, 2); % downsample plot data
headers = {'time', 'velocity_x', 'velocity_y', 'roll', 'pitch', 'yaw', 'roll_ref', 'pitch_ref', 'yaw_ref', 'q0', 'q1', 'q2', 'q3', 'omega_body_x', 'omega_body_y', 'omega_body_z', 'omega_body_ref_x', 'omega_body_ref_y', 'omega_body_ref_z', 's0', 's1', 's2', 'tau0', 'tau1', 'tau2'};
csvwrite_with_headers('sine_roll_test_system.csv', out, headers);