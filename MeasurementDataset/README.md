...
The dataset includes both logged data from the embedded firmware, including raw sensor measurements and processed estimates and controller outputs. The dataset also includes Vicon-based ground truth position measurements.

The test set is used to test and validate the developed algorithms. All plots in the thesis are based on this test dataset generated using the Processing scripts found within each test subfolder.

Use MATLAB to parse and visualize the logs with the [`LoadDump.m`](https://github.com/mindThomas/Kugle-MATLAB/blob/master/DataProcessing/functions/LoadDump.m) and [`VisualizeDump.m`](https://github.com/mindThomas/Kugle-MATLAB/blob/master/MeasurementDataset/VisualizeDump.m) scripts.

## Vicon origin
The Vicon origin is positioned in between the two bottom holes (of the four holes on the side panels close to the tablet).

Left hole pos (from bottom of ball)
`-9.9, 74, 1040.6 mm`

Right hole pos (from bottom of ball)
`-9.9, -74, 1040.6 mm`

Ball center (height above ground)
129 mm

From the center of the ball to center of Vicon origin
`-9.9, 0, 911.6 mm`
