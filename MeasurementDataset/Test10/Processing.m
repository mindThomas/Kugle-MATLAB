...
% good disturbance test: 48.34 to 78.34
offset = 48.34;
length = 30;
% reaching phase: 0 to 14 (or 46 for longer)
%offset = 0;
%length = 46;
[data, vicon] = TrimSynced(data, vicon, 27+offset, length); % full = 27 length 80
...
disturbance_times = [6, 10.25, 11.69, 17.8, 24.74, 29.40, 37.17, 38.43, 44.17, 51.71, 55.91, 60.34, 64.66, 65.82, 73.08, 79.21];
...
csvwrite_with_headers('zero_balance_agressive_vicon1.csv', out, headers);

out = [data.time, data.velocity, rad2deg(data.roll), rad2deg(data.pitch), rad2deg(data.yaw-yaw_ref(end)), data.q, data.S, data.torque];
out = Downsample(out, 2); % downsample plot data
headers = {'time', 'velocity_x', 'velocity_y', 'roll', 'pitch', 'yaw', 'q0', 'q1', 'q2', 'q3', 's0', 's1', 's2', 'tau0', 'tau1', 'tau2'};
csvwrite_with_headers('zero_balance_agressive_system1.csv', out, headers);

% Store disturbance times
csvwrite_with_headers('zero_balance_agressive_disturbances1.csv', {disturbance_times'}, {'time'});