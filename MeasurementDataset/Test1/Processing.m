...
[data, vicon] = TrimSynced(data, vicon, 60+25, 22);
...
ax1 = subplot(2,1,1); plot(vicon.time, vicon.velocity(:,1), data.time, velocity_kinematics_mti(:,1)); ylabel('m/s'); title('dx'); legend('Vicon', 'Kinematics');
ax2 = subplot(2,1,2); plot(vicon.time, vicon.velocity(:,2), data.time, velocity_kinematics_mti(:,2)); ylabel('m/s'); title('dy'); legend('Vicon', 'Kinematics');
linkaxes([ax1,ax2],'x');
xlabel('Time [s]');
...
ax3 = subplot(3,1,3); plot(vicon.time, rad2deg(vicon.yaw), data.time, rad2deg(data.yaw), data.time, rad2deg(data.mti_yaw)); ylabel('Angle [deg]'); title('Yaw');
