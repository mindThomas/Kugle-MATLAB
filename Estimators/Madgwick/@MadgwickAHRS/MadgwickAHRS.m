...
%   28/09/2011    SOH Madgwick    Initial release
    ...
    methods (Access = public)
        ...
        function obj = Update(obj, Gyroscope, Accelerometer, Magnetometer)
            ...
            dq = 0.5 * quaternProd(q, [0 Gyroscope(1) Gyroscope(2) Gyroscope(3)]) - obj.Beta * step';

            % Integrate to yield quaternion
            q = q + dq * obj.SamplePeriod;
            obj.Quaternion = q / norm(q); % normalise quaternion
			obj.QuaternionDerivative = dq;
        end
        function obj = UpdateIMU(obj, Gyroscope, Accelerometer)
            ...
            else % if the accelerometer measurement is not valid, use gyroscope to only propagate the quaternion
                step = [0;0;0;0];
            end
